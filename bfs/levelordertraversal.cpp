...
template <typename T> auto get_words() -> std::vector<T> {
  std::string line;
...
template <typename T> auto put_words(const std::vector<T> &v) -> void {
  if (!v.empty()) {
...
auto main() -> int {
  ...
  Node<int> *root =
      build_tree<int>(root_it, [](auto s) { return std::stoi(s); });
